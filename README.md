...
## Summary
In summary there are most likely other features that when added to the dataset could enhance and improve the machine learning component to better predict severe-to-worse-drought for the Colorado River Basin.  In particular, it would be ideal to find features that could improve the recall (sensitivity) value for predicting severe-to-worse-drought because even when predicted true with an actual false outcome it would trigger the region to conserve water usage as a reaction to the prediction.  The Colorado River Basin is a complex region with high population growth in many of the metro areas as well as expansive agriculture areas that both depend on and use the river water.  Having a solid drought prediction model would better prepare the region for water conservation action during periods of drought or non-drought.
